...
### Bug 1:
in `index.css`, fix property `'display:fixed'` of `.RampInputSelect--dropdown-container`  to `'display:absolute'`

### Bug 2:
add `htmlFor={inputId}` in the label element in `InputCheckBox/index.tsx`
...
```
at `loadTransactionsByEmployee()`, so that if employee id is empty, it fetches for all transactions instead of trying to fetch transaction by employees.

### Bug 4:
in `setPaginatedTransactions() ` of `usePaginatedTransactions.ts`, if paginatedTransactions.data exists, concatenate new transaction data to previous transactions instead of replacing it.

### Bug 5:
in `loadAllTransactions()` of `App.tsx`, change position of `setIsLoading(false)` to right after `await employeeUtils.fetchAll()`, so that it clears the loading text of inputselect right after fetching employee data, and don`t have to wait for transaction data fetching to complete.

### Bug 6:
in `App.tsx`, change property 'disabled' of `view more` button to `paginatedTransactionsUtils.loading || !paginatedTransactions || !paginatedTransactions.nextPage`, so that the button is disabled when next page is null.
